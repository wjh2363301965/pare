...
//        }
   ...
   /* *//** 判断文件格式*//*
    public static RespData<Boolean> fileType(MultipartFile file, String... type) {
        RespData<Boolean> respData = new RespData<>();
        if (file.isEmpty()) {
            respData.setRespCode(RespCodeEnum.FAIL);
            respData.setReason("上传失败，文件内容为空!");
            return respData;
        }
        // 判断文件类型
        ...
        if (StringUtils.isBlank(name)) {
            respData.setRespCode(RespCodeEnum.FAIL);
            respData.setReason("上传失败，请输入文件名!");
            return respData;
        }
        if (!name.contains(".")) {
            respData.setRespCode(RespCodeEnum.FAIL);
            respData.setReason("上传失败，文件名格式不正确!");
            return respData;
        }
        if (type == null) {
            // 对格式无要求
        ...
        if (!strings.contains(fileType)) {
            respData.setRespCode(RespCodeEnum.FAIL);
            respData.setReason("文件格式不正确，请上传."+fileType+"文件!");
            return respData;
        }
        respData.setRespCode(RespCodeEnum.SUCCESS);
        return respData;
